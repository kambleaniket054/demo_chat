...
import 'package:demo_chat/vido_edit_new/dao/project_dao.dart';
import 'package:demo_chat/vido_edit_new/service/project_service.dart';
...
class DirectorService {
  ...
  add(AssetType assetType) async {
    ...
    if (assetType == AssetType.video) {
      FilePickerResult fileresult = await FilePicker.platform.pickFiles(type: FileType.video,allowMultiple: true);
      if (fileresult != null) {
        filePaths = fileresult.paths.map((path) => File(path)).toList();
      }
      ...
      List<File> fileList = filePaths/*_sortFilesByDate(filePaths)*/;
      ...
      FilePickerResult fileresult = await FilePicker.platform.pickFiles(type: FileType.image,allowedExtensions: ["mp4"],allowMultiple: true);
      if (fileresult != null) {
        filePaths = fileresult.paths.map((path) => File(path)).toList();
      }
      if (filePaths == null) {
      ...
      FilePickerResult fileresult = await FilePicker.platform.pickFiles(type: FileType.audio,allowMultiple: true);
      if (fileresult != null) {
        filePaths = fileresult.paths.map((path) => File(path)).toList();
      }
      if (filePaths == null) {
  ...
  _addAssetToLayer(int layerIndex, AssetType type, String srcPath) async {
   ...
   await layerPlayers[layerIndex]?.addMediaSource(
